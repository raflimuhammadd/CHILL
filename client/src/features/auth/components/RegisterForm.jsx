...
const validateForm = (formData) => {
    ...
    if (!formData.password) {
        errors.password = 'Kata sandi diperlukan';
    } else if (formData.password.length < 6) {
        errors.password = 'Kata sandi minimal 6 karakter';
    }

    if (!formData.confirmPassword) {
        errors.confirmPassword = 'Konfirmasi kata sandi diperlukan';
...
function RegisterForm() {
    ...
    return (
        <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <FormField label="Email (opsional)" htmlFor="reg-email" error={errors.email}>
                <Input
                    id="reg-email"
                    name="email"
                    type="email"
                    ...
                    onChange={handleChange}
                    placeholder="Masukkan email (opsional, untuk verifikasi)"
                    error={errors.email}
                />
            </FormField>
            ...
            <div className="space-y-3">
                {error && (
                    <div className="bg-red-500/20 border-red-500 text-red-500 px-4 py-3 rounded-lg text-center text-sm">
                        {error}
                    </div>
                )}
